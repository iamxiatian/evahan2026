...
def compute_metrics(ref: str, hyp: str) -> dict:
    ...
    precision = (
        correct_chars / hyp_len
        if hyp_len > 0
        else (1.0 if ref_len == 0 else 0.0)
    )
    recall = (
        correct_chars / ref_len
        if ref_len > 0
        else (1.0 if hyp_len == 0 else 0.0)
    )
    f1 = (
    ...
    with open(file_path, encoding="utf-8") as f:
        for line in f:
            ...
            try:
                ...
                res = compute_metrics(
                    data.get("text", ""), data.get("predicted", "")
                )
        ...
        for line in f:
            ...
            try:
                ...
                hyp_norm = normalize_text(
                    data.get("predicted", ""), variant_map
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="EvaHan Double Evaluation System"
    )
    parser.add_argument(
        "--file", type=str, required=True, help="Prediction JSONL file"
    )
    parser.add_argument(
        "--variant",
        type=str,
        default="variant_data.json",
        help="Variant map JSON file",
    )
    ...
    if os.path.exists(args.file):
        ...
        # 2. 运行异体字归一化评估
        print(
            ">>> 2. 正在进行 [异体字归一化评估(variant_data.json)] (Variant Evaluation)..."
        )
