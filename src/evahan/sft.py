...
# model
model_id_or_path = config.QWEN_VL_7B_INSTRUCT
...
train_dataset = LazyLLMDataset(
    train_dataset, template.encode, random_state=data_seed
)
val_dataset = LazyLLMDataset(
    val_dataset, template.encode, random_state=data_seed
)
...
plot_images(
    images_dir, training_args.logging_dir, ["train/loss"], 0.9
)  # save images
