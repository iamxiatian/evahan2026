...
def load_json_to_dict(json_path: str) -> dict[str, str]:
    ...
    if not isinstance(data, list):
        raise ValueError(
            f"The JSON file should be in array format, currently as follows:{type(data)}"
        )
    ...
    for idx, sample in enumerate(data):
        if (
            ...
            or "image_path" not in sample
            or "text" not in sample
        ):
            raise ValueError(
                f"The {idx + 1}th data entry has a format error and must include the image_path and text fields:{sample}"
            )
        ...
        if img_path in result_dict:
            ...
            print(
                f"The image path {img_path} appears multiple times within {json_path}. Retrieve the text from the last occurrence."
            )
    ...
    if duplicate_images:
        print(
            f"Note: {len(duplicate_images)} duplicate image paths detected: {list(duplicate_images)}"
        )
...
def evaluate_ocr(ref_json_path: str, pred_json_path: str) -> dict:
    ...
    if missing_in_pred:
        print(
            f"Warning: {len(missing_in_pred)} reference images are missing in the prediction file. This is treated as an empty prediction."
        )
    ...
    if extra_in_pred:
        print(
            f"Note: {len(extra_in_pred)} predicted images are not in the reference set and will be ignored:"
        )
    ...
    for img_path in ref_images:
        ...
        hyp_text = pred_dict.get(
            img_path, ""
        )  # Missing predictions are treated as empty text.
    ...
    overall_metrics = {
        "overall_cer": round(total_cer / total_samples, 4)
        if total_samples > 0
        else 0.0,
        "overall_precision": round(total_precision / total_samples, 4)
        if total_samples > 0
        else 0.0,
        "overall_recall": round(total_recall / total_samples, 4)
        if total_samples > 0
        else 0.0,
        "overall_f1": round(total_f1 / total_samples, 4)
        if total_samples > 0
        else 0.0,
        "overall_ned": round(total_ned / total_samples, 4)
        if total_samples > 0
        else 0.0,
        "overall_comprehensive_score": round(
            total_comprehensive / total_samples, 4
        )
        if total_samples > 0
        else 0.0,
    ...
    error_stats = {
        ...
        "missing_ratio": round(total_missing / total_errors, 4)
        if total_errors > 0
        else 0.0,
        "extra_ratio": round(total_extra / total_errors, 4)
        if total_errors > 0
        else 0.0,
        "wrong_ratio": round(total_wrong / total_errors, 4)
        if total_errors > 0
        else 0.0,
    }
...
def print_eval_result(eval_result: dict):
    ...
    print("\n" + "=" * 80)
    print(
        "                    Comprehensive Evaluation Report on OCR for Ancient Texts                    "
    )
    print("=" * 80)
    ...
    print(
        f"├─ Number of missing samples to be predicted: {data_check['missing_in_pred']}"
    )
    print(
        f"└─ Predicting the number of redundant samples: {data_check['extra_in_pred']}"
    )

    overall = eval_result["overall_metrics"]
    print("\n[2. Core Evaluation Metrics]")
    print(
        f"├─ Comprehensive Score (CER50%+F130%+NED20%): {overall['overall_comprehensive_score']:.4f}"
    )
    ...
    print(
        f"├─ Total Correct Characters: {overall['total_correct_chars']}/{overall['total_ref_chars']}"
    )
    ...
    print(
        f"├─ Total missing characters (deleted): {errors['total_missing']} ({errors['missing_ratio']:.2%})"
    )
    print(
        f"├─ Total number of characters (inserted): {errors['total_extra']} ({errors['extra_ratio']:.2%})"
    )
    print(
        f"└─ Total number of typos (replaced): {errors['total_wrong']} ({errors['wrong_ratio']:.2%})"
    )
    ...
    for i, sample in enumerate(eval_result["sample_details"][:5]):
        ...
        print(
            f"├─ Reference Text: {sample['ref_text']} (length: {sample['ref_length']})"
        )
        print(
            f"├─ Predictive Text: {sample['pred_text']} (length: {sample['pred_length']})"
        )
        print(
            f"├─ Core Evaluation Metrics: CER={sample['cer']} | Comprehensive Score={sample['comprehensive_score']}"
        )
        print(
            f"├─ Supplementary indicators: F1={sample['f1']} | NED={sample['ned']} | precision={sample['precision']} | recall={sample['recall']}"
        )
        print(
            f"└─ Error Details: Missing character={sample['missing_chars']} | Extra characters={sample['extra_chars']} | Wrong characters={sample['wrong_chars']}"
        )
...
if __name__ == "__main__":
    ...
    PRED_JSON_PATH = (
        "/test_data/Dataset_A_test.json"  # Prediction Data JSON Path
    )
