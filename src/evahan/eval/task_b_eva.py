...
class BBox:
    ...
    def __init__(self, points: list[list[int]], label: str):
        ...
        self.area = (self.rect[2] - self.rect[0]) * (
...
class LayoutEvaluator:
    ...
    def match_single_image(
        self,
        pred_boxes: list[tuple[BBox, str]],
        gt_boxes: list[tuple[BBox, str]],
    ) -> dict:
        """
        ...
        for label, gt_list in gt_by_label.items():
            ...
            for i in np.argsort(
                -iou_matrix.max(axis=1) if len(pred_list) > 0 else []
            ):
    ...
    def calculate_ap(
        self, pred_boxes: list[BBox], gt_boxes: list[BBox], iou_th: float
    ) -> float:
        """
        ...
        for t in np.linspace(0, 1, 11):
            ...
            p_interp = [
                p for r, p in zip(recall_list, precision_list) if r >= t
            ]
    ...
    def evaluate(self, pred_json: str, gt_json: str) -> dict:
        ...
        if self.match_ious:
            ...
            print(
                f"Count with IoU >= 0.5: {sum(1 for i in self.match_ious if i >= 0.5)}"
            )
            print(
                f"Count with IoU >= 0.3: {sum(1 for i in self.match_ious if i >= 0.3)}"
            )
            print(
                f"Count with IoU < 0.3: {sum(1 for i in self.match_ious if i < 0.3)}"
            )
        ...
        for iou_th in iou_thresholds:
            ...
            for label in set(pred_boxes_by_label.keys()) | set(
        ...
        print(
            f"Micro F1:       {micro_f1:.4f} (P={micro_precision:.4f}, R={micro_recall:.4f})"
        )
        ...
        print(
            f"{'Label':<15} {'TP':>6} {'FP':>6} {'FN':>6} {'Prec':>6} {'Rec':>6} {'F1':>6}"
        )
        print("-" * 60)
        for label in sorted(label_stats.keys()):
            ...
            print(
                f"{label:<15} {tp:>6} {fp:>6} {fn:>6} {prec:>6.3f} {rec:>6.3f} {f1:>6.3f}"
            )
