...
class Evaluator:
    ...
    def __predict_folder(
        ...
        if resume and out_jsonl.exists():
            predicated_images = Evaluator.__read_predicted(out_jsonl)
    ...
    def run(
        ...
        for in_jsonl, out_json, task_type in zip(
            ...
            Evaluator.to_evahan_format(in_jsonl, out_json, task_type) 

        logger.info("All done!")


def run_trainset(run_name: str) -> None:
...
def run_testset(run_name: str) -> None:
