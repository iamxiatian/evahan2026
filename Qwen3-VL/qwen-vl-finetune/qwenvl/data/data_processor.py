...
from .ocr_rec_augment.rec_aug import SVTRAug, PARSeqAug
...
rec_aug = SVTRAug()
det_aug = PARSeqAug()
temp_dir = "/mnt/public/lyl/Qwen3-VL/temp"
if not os.path.exists(temp_dir):
    os.makedirs(temp_dir)
def rec_aug_image(image_path: str)->str:
    image = cv2.imread(image_path)
    image = cv2.rotate(image, cv2.ROTATE_90_CLOCKWISE)
    data = {
        'image': image,
    }
    aug_data = rec_aug(data)
    aug_image = aug_data['image']
    aug_image = cv2.rotate(aug_image, cv2.ROTATE_90_COUNTERCLOCKWISE)
    save_path = os.path.join(temp_dir, f"{uuid.uuid4()}.jpg")
    cv2.imwrite(save_path, aug_image)
    return save_path

def det_aug_image(image_path: str)->str:
    image = cv2.imread(image_path)
    data = {
        'image': image,
    }
    aug_data = det_aug(data)
    aug_image = aug_data['image']
    save_path = os.path.join(temp_dir, f"{uuid.uuid4()}.jpg")
    cv2.imwrite(save_path, aug_image)
    return save_path
...
def _build_messages(item: Dict[str, Any], base_path: Path) -> List[Dict[str, Any]]:
    ...
    image_pool = []
    for img in images:
        dataset_prefix = str(img).split("/")[-2]
        assert dataset_prefix in ["Dataset_A", "Dataset_B", "Dataset_C"], f"dataset_prefix: {dataset_prefix} not supported"
        if dataset_prefix == "Dataset_B":
            image_new_path = det_aug_image(str(_make_abs_paths(base_path, img)))
        else:
            print(f"dataset_prefix: {dataset_prefix} not supported")
            image_new_path = rec_aug_image(str(_make_abs_paths(base_path, img)))
        image_pool.append({"type": "image", "image": image_new_path})
